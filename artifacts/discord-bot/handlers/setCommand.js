...
function mainMenuRows(categories) {
  ...
  const row2 = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      ...
      .setStyle(ButtonStyle.Success),
    new ButtonBuilder()
      .setCustomId('set_new_panel')
      .setLabel('🔁 Switch Panel')
      .setStyle(ButtonStyle.Secondary),
  );
...
// ─── Emoji picker (search-based, no pages) ───────────────────────────────────

function buildEmojiPickerComponents(catId, guild) {
  // Now search-based — returns a search button + skip button
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`set_emoji_search_add:${catId}`)
        .setLabel('🔍 Search emoji by name')
        .setStyle(ButtonStyle.Primary),
      new ButtonBuilder()
        .setCustomId(`set_skip_emoji:${catId}`)
        .setLabel('Skip (no emoji)')
        .setStyle(ButtonStyle.Secondary),
    ),
  ];
}

// ─── Emoji picker for editing (search-based) ─────────────────────────────────
...
function buildEditEmojiPickerRows(catId, optIndex, guild, page) {
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`set_emoji_edit_search:${catId}:${optIndex}`)
        .setLabel('🔍 Search emoji by name')
        .setStyle(ButtonStyle.Primary),
      new ButtonBuilder()
        .setCustomId(`set_no_emoji_edit:${catId}:${optIndex}`)
        .setLabel('No Emoji')
        .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
...
// ─── Category icon picker (search-based) ─────────────────────────────────────

function buildCatIconPickerRows(catId, guild, page) {
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`set_cat_icon_search:${catId}`)
        .setLabel('🔍 Search emoji by name')
        .setStyle(ButtonStyle.Primary),
      new ButtonBuilder()
        .setCustomId(`set_no_cat_icon:${catId}`)
        .setLabel('No Icon')
        .setStyle(ButtonStyle.Secondary),
      new ButtonBuilder()
        .setCustomId(`set_back_edit:${catId}`)
...
async function handleSetButton(interaction, dynamicRoles, saveStorage) {
  ...
  // ── New Panel ──
  if (id === 'set_new_panel') {
    // Switch to a different panel (or go back to picker)
    const panels = dynamicRoles.panels ?? [];
    if (panels.length <= 1) {
      await interaction.update({
        embeds: [mainMenuEmbed(dynamicRoles.categories, dynamicRoles)
          .setFooter({ text: 'Only one panel exists. Use /panel to create more.' })],
        components: mainMenuRows(dynamicRoles.categories),
      });
      return;
    }
    const switchButtons = panels.map(p =>
      new ButtonBuilder()
    ...
    for (let i = 0; i < switchButtons.length; i += 5) {
      switchRows.push(new ActionRowBuilder().addComponents(switchButtons.slice(i, i + 5)));
    }
    switchRows.push(new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('set_back_main').setLabel('← Cancel').setStyle(ButtonStyle.Secondary)
    ));
    await interaction.update({
      embeds: [new EmbedBuilder().setTitle('🔁 Switch Panel').setDescription('Select which panel to configure:').setColor(0x5865F2)],
  ...
  if (id.startsWith('set_emoji_edit_DISABLED:')) {
    const isPrev = false;
    const rest = '';
  ...
  if (id.startsWith('set_retry_emoji:')) {
    ...
    // Actually skip — add the option with no emoji
    pendingOptions.delete(key);
    cat.options.push({ label: pending.label, emoji: null, roleId: pending.roleId });
    saveStorage();
    await interaction.update({
      embeds: [categoryEditorEmbed(cat, `✅ Added **${pending.label}** (no emoji).`)],
      components: categoryEditorRows(catId, cat),
    });
    return;
  }

  // ── Search emoji for add flow ──
  if (id.startsWith('set_emoji_search_add:')) {
    const catId = id.slice('set_emoji_search_add:'.length);
    const modal = new ModalBuilder()
    ...
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder()
        ...
        .setLabel('Type emoji name (e.g. star, heart, fire)')
        .setStyle(TextInputStyle.Short)
        .setRequired(true)
        .setMaxLength(50)
        .setPlaceholder('e.g. game, heart, fire...')
    ));
    await interaction.showModal(modal);
    return;
  }

  // ── Search emoji for edit flow ──
  if (id.startsWith('set_emoji_edit_search:')) {
    const rest = id.slice('set_emoji_edit_search:'.length);
    const lastColon = rest.lastIndexOf(':');
    const catId = rest.slice(0, lastColon);
    const optIndexStr = rest.slice(lastColon + 1);
    const modal = new ModalBuilder()
      .setCustomId(`modal_search_emoji_edit:${catId}:${optIndexStr}`)
      .setTitle('🔍 Search Emoji');
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder()
        .setCustomId('emoji_search')
  ...
  // ── Remove emoji from existing option ──
  if (id.startsWith('set_no_emoji_edit:')) {
    const rest = id.slice('set_no_emoji_edit:'.length);
    const lastColon = rest.lastIndexOf(':');
    const catId = rest.slice(0, lastColon);
    const optIndex = parseInt(rest.slice(lastColon + 1));
    const cat = categories.find(c => c.id === catId);
    const opt = cat?.options[optIndex];
    if (opt) { opt.emoji = null; saveStorage(); }
    await interaction.update({
      embeds: [categoryEditorEmbed(cat, `✅ Emoji removed from **${opt?.label ?? 'option'}**.`)],
      components: categoryEditorRows(catId, cat),
    });
    return;
  }
  ...
  // ── Search cat icon ──
  if (id.startsWith('set_cat_icon_search:')) {
    const catId = id.slice('set_cat_icon_search:'.length);
    const modal = new ModalBuilder()
      .setCustomId(`modal_search_cat_icon:${catId}`)
      .setTitle('🔍 Search Category Icon');
    modal.addComponents(new ActionRowBuilder().addComponents(
      new TextInputBuilder()
        .setCustomId('emoji_search')
        .setLabel('Type emoji name (e.g. star, heart, fire)')
  ...
  // ── Remove cat icon ──
  if (id.startsWith('set_no_cat_icon:')) {
    const catId = id.slice('set_no_cat_icon:'.length);
    const cat = categories.find(c => c.id === catId);
    if (cat) { cat.icon = null; saveStorage(); }
    await interaction.update({
      embeds: [categoryEditorEmbed(cat, `✅ Icon removed from **${cat?.name ?? 'category'}**.`)],
      components: categoryEditorRows(catId, cat),
    });
    return;
  }
...
async function handleSetModal(interaction, dynamicRoles, saveStorage) {
  ...
  if (id.startsWith('modal_add_option:')) {
    ...
    let rawRoleInput = interaction.fields.getTextInputValue('opt_role_id').trim() || null;
    // Strip Discord mention format: <@&roleId> → roleId
    if (rawRoleInput) {
      const mentionMatch = rawRoleInput.match(/^<@&(\d+)>$/);
      if (mentionMatch) rawRoleInput = mentionMatch[1];
    }
    let roleId = rawRoleInput || null;

    // If role ID was provided, validate it actually exists
    if (roleId) {
      try {
        const foundRole = await interaction.guild.roles.fetch(roleId).catch(() => null);
        if (!foundRole) {
          await interaction.reply({
            content: `❌ Role ID \`${roleId}\` not found on this server. Make sure you copied the correct ID.`,
            ephemeral: true,
          });
          return;
        }
  ...
  // ── Search emojis for EDIT flow ──
  if (id.startsWith('modal_search_emoji_edit:')) {
    const rest2 = id.slice('modal_search_emoji_edit:'.length);
    const lastColon2 = rest2.lastIndexOf(':');
    const catId2 = rest2.slice(0, lastColon2);
    const optIndex2 = parseInt(rest2.slice(lastColon2 + 1));
    const cat2 = categories.find(c => c.id === catId2);
    const searchTerm2 = interaction.fields.getTextInputValue('emoji_search').trim().toLowerCase();
    if (!searchTerm2) { await interaction.reply({ content: '❌ Please enter a search term.', ephemeral: true }); return; }
    await interaction.guild.emojis.fetch().catch(() => {});
    const matching2 = [...interaction.guild.emojis.cache.values()].filter(e => e.name.toLowerCase().includes(searchTerm2)).slice(0, 24);
    if (matching2.length === 0) { await interaction.reply({ content: `❌ No emojis found matching **"${searchTerm2}"**.`, ephemeral: true }); return; }
    const opts2 = [
      new StringSelectMenuOptionBuilder().setLabel('No Emoji').setValue('none').setDescription('Remove emoji from this option'),
      ...matching2.map(e => new StringSelectMenuOptionBuilder()
    ...
    const sel2 = new StringSelectMenuBuilder()
      .setCustomId(`set_pick_emoji_edit:${catId2}:${optIndex2}:0`)
      .setPlaceholder(`${matching2.length} emoji(s) found for "${searchTerm2}"...`)
      .addOptions(opts2);
    await interaction.update({
      embeds: [new EmbedBuilder().setTitle('🔍 Emoji Search Results').setDescription(`Found **${matching2.length}** emoji(s) for **"${searchTerm2}"**`).setColor(0x57F287)],
      components: [
        new ActionRowBuilder().addComponents(sel2),
        new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`set_back_edit:${catId2}`).setLabel('← Cancel').setStyle(ButtonStyle.Secondary)),
      ],
    });
    return;
  }

  // ── Search emojis for cat icon ──
  if (id.startsWith('modal_search_cat_icon:')) {
    const catId3 = id.slice('modal_search_cat_icon:'.length);
    const cat3 = categories.find(c => c.id === catId3);
    const searchTerm3 = interaction.fields.getTextInputValue('emoji_search').trim().toLowerCase();
    if (!searchTerm3) { await interaction.reply({ content: '❌ Please enter a search term.', ephemeral: true }); return; }
    await interaction.guild.emojis.fetch().catch(() => {});
    const matching3 = [...interaction.guild.emojis.cache.values()].filter(e => e.name.toLowerCase().includes(searchTerm3)).slice(0, 24);
    if (matching3.length === 0) { await interaction.reply({ content: `❌ No emojis found matching **"${searchTerm3}"**.`, ephemeral: true }); return; }
    const opts3 = [
      new StringSelectMenuOptionBuilder().setLabel('No Icon').setValue('none').setDescription('Use no icon for this category'),
      ...matching3.map(e => new StringSelectMenuOptionBuilder()
        .setLabel(e.name.slice(0, 25))
        .setValue(e.animated ? `<a:${e.name}:${e.id}>` : `<:${e.name}:${e.id}>`)
        .setEmoji({ id: e.id, name: e.name, animated: e.animated })
      ),
    ];
    const sel3 = new StringSelectMenuBuilder()
      .setCustomId(`set_pick_cat_icon:${catId3}:0`)
      .setPlaceholder(`${matching3.length} emoji(s) found...`)
      .addOptions(opts3);
    await interaction.update({
      embeds: [new EmbedBuilder().setTitle('🔍 Icon Search Results').setDescription(`Found **${matching3.length}** emoji(s) for **"${searchTerm3}"**`).setColor(0x57F287)],
      components: [
        new ActionRowBuilder().addComponents(sel3),
        new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`set_back_edit:${catId3}`).setLabel('← Cancel').setStyle(ButtonStyle.Secondary)),
      ],
    });
    return;
  }

  // ── Search emojis by name (add flow) ──
